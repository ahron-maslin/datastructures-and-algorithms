...
int main() {
  srand(time(NULL));
  min_heap_t* heap = array_init(100);
  ...
  build_heap_min(heap2);
  
  clock_t end2 = clock();
  
  assert(is_min_heap(heap2->array, size(heap2), 0));
  ...
  // testing with inserting with heapify using build heap min O(n)
  min_heap_t* heap3 = array_init(1);
  ...
  assert(is_min_heap(heap3->array, size(heap3), 0));
  ...
  assert(is_min_heap(heap4->array, size(heap4), 0));
